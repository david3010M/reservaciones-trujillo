...
export default function TablaHabitacionesDisponible({
  room,
  date,
  dateTo,
}: Props) {
  ...
  const handleReserva = (habitacion: number) => {
    ...
    router.push(
      `/habitaciones/${date}/${dateTo}/${room.tipohabitacion.id}/reservar`
    );
  };
  ...
  return (
    <div className="w-full space-y-10">
      {Object.entries(habitacionesPorPiso).map(([piso, habitaciones]) => (
        <div key={piso}>
          <h2 className="text-2xl font-bold text-hotel-brown mb-4">{piso}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {habitaciones.map((habitacion) => (
              ...
              >
                ...
                <Button
                  className="bg-hotel-brown mt-2 ripple-xl"
